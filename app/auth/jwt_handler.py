...
def create_access_token(
    data_to_encode: dict, expire_delta: timedelta | None = None
) -> str | dict[str, JWTError]:
    ...
    to_encode = (
        data_to_encode.copy()
    )  ## on realise une copy des elements avec lesquels on veut travailler
    ...
    to_encode.update(
        {"exp": expiration}
    )  ## met a jour pour ajouter une noyvelle info: le tmps d'expiration du token
