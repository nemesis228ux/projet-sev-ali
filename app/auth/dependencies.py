...
def get_current_user(
    token: Annotated[str, Depends(oauth_schema)], db: Session = Depends(get_db)
) -> User:
    ...
    user_id = claims_data.get(
        "sub"
    )  ## on avait use user_id pr encoder donc on peut le recuperer ici en str
    ...
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Cet utilisateur n'existe pas"
        )
