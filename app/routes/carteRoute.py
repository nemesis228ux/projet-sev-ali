...
from app.crud.carteCrud import (
    get_all_cartes,
    create_new_carte,
    get_user_cartes,
    get_user_cartes_in_an_account,
    get_user_specific_carte,
    delete_user_carte,
    change_carte_password,
)
from app.models.carte import CarteTypes
from app.schemas.baseSchema import ApiBaseResponse
from app.schemas.carteSchema import (
    CartesView,
    CarteView,
    CreateCarte,
    ChangeCartePassword,
...
def get_all_cartes_in_bd(
    carte_type: Optional[CarteTypes] = Query(
...
def create_a_new_carte(
    ...
    result = create_new_carte(
        bd_session, user_id, body.account_id, body.carte_type, body.password
    )
...
def get_user_all_cartes(
    carte_type: Optional[CarteTypes] = Query(
        None, description="Param de requete otionnelle pour filtrer les cartes par type"
    ),
    account_id: Optional[int] = Query(
        None,
        description="Param de requète optionnel pour obtenir uniquement les cartes d'un compte précis",
    ),
    user_id: int = Depends(get_current_user_id),
    bd_session: Session = Depends(get_db),
) -> CartesView:
    ...
    if account_id:
        result = get_user_cartes_in_an_account(
            bd_session, user_id, account_id, carte_type
        )
    else:
...
def modify_carte_password(
    ...
    result = change_carte_password(
        bd_session, user_id, body.carte_id, body.old_password, body.new_password
    )
