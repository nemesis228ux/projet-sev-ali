...
from app.crud.userCrud import (
    get_user_using_email,
    get_user_using_id,
    get_users,
    update_user_using_email,
    delete_user,
)
...
@router.get(
    "/email",
    dependencies=[Depends(get_current_user)],
    response_model=ApiBaseResponse[UserRead],
)
...
@router.delete(
    "/{user_id}", dependencies=[Depends(isAdmin)], response_model=ApiBaseResponse[str]
)
def delete_user_by_id(
    ...
    return ApiBaseResponse.success_response(
        f"Utisateur {deleted_user.nom_user} supprimé"
    )
